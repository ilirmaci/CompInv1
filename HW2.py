...
def findEvents(symbols, startday,endday, marketSymbol,verbose=False):
	...
	for symbol in symbols:
	    ...
	    for i in range(2,len(close[symbol])):
	        if close[symbol][i-1]>=5.0 and close[symbol][i]<5: #TRUE if price drops below 5.0
             		np_eventmat[symbol][i] = 1.0  #overwriting by the bit, marking the event
			
	return np_eventmat


def writeOrder(date, symbol, operation, quantity, filename):
        '''(datetime, str, str, int, str) -> NullType
        Writes order into specified filename in CSV format in
        the following order:
